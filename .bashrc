	...
	for archive in $*; do
		if [ -f $archive ] ; then
			case $archive in
				...
				*.7z)        7z x $archive        ;;
				*)           echo "je ne sais pas comment unzip : '$archive'..." ;;
			esac
		else
			echo "'$archive' n'est pas un fichier valide"
		fi
	done
}
...
function __setprompt
{
	...
	PS1+="(\[${GREEN}\]$(ls -lah | /bin/grep -m 1 total | /bin/sed 's/total //')\[${DARKGRAY}\]:"

	# Number of files
	PS1+="\[${GREEN}\]\$(ls -A -1 | /usr/bin/wc -l)\[${DARKGRAY}\])"
